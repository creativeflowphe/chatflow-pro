...
-- Criar tabela de contatos com campo auto_reply_enabled
...
-- Adicionar coluna auto_reply_enabled se a tabela já existir
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'contacts' AND column_name = 'auto_reply_enabled'
  ) THEN
    ALTER TABLE contacts ADD COLUMN auto_reply_enabled boolean DEFAULT true;
  END IF;
END $$;
...
-- Criar tabela de mensagens com campo is_automated
...
-- Adicionar coluna is_automated se a tabela já existir
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'messages' AND column_name = 'is_automated'
  ) THEN
    ALTER TABLE messages ADD COLUMN is_automated boolean DEFAULT false;
  END IF;
END $$;
