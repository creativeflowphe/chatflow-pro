...
export const EditKeywordModal = ({ keyword, isOpen, onClose, onSuccess }: EditKeywordModalProps) => {
  ...
  useEffect(() => {
    if (keyword) {
      ...
      setKeywords(keyword.keywords.length > 0 ? keyword.keywords : ['']);
      setMatchType(keyword.match_type as any);
      setCaseSensitive(keyword.case_sensitive);
      setReplyType(keyword.reply_type as any);
      setReplyMessage(keyword.reply_message || '');
      setTags(keyword.tags.length > 0 ? keyword.tags : ['']);
      setPriority(keyword.priority);
    }
  }, [keyword]);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200 sticky top-0 bg-white">
          <h2 className="text-xl font-semibold text-gray-900">Editar Palavra-chave</h2>
          <button
            onClick={onClose}
